...
class TestGetMode(unittest.TestCase):
    ...
    def test_sql_path_error_from_execute_raises_exception(self):
        ...
        with mock.patch.dict(os.environ, {"SKIP_MODE_CHECK_NON_OCI_MYSQL": "false"}), \
            ...
            with self.assertRaises(Exception) as ctx:
                m._get_mode("any_conn")
            self.assertIn("Exception occurred while fetching cloud provider", str(ctx.exception))
    ...
    def test_sql_path_case_insensitive_provider(self):
        """Mode.from_string should handle case-insensitive provider strings like 'lcl' or 'oci'."""
